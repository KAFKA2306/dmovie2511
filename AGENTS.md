...
## 動画生成検証手順
動画生成を検証する前に、Wan Q5 量子化版（`QuantStack/Wan2.2-Animate-14B-GGUF` → `Wan2.2-Animate-14B-Q5_K_M.gguf`）を Hugging Face から取得し `ComfyUI/models/diffusion_models/Wan2.2-Animate-14B-Q5_K_M.gguf` に配置します。`pip install "huggingface_hub[cli]"` → `huggingface-cli download QuantStack/Wan2.2-Animate-14B-GGUF Wan2.2-Animate-14B-Q5_K_M.gguf --local-dir ComfyUI/models/diffusion_models` で直接配置可能です。テキストエンコーダ（`ComfyUI/models/text_encoders/umt5_xxl_fp8_e4m3fn_scaled.safetensors`）と VAE（`ComfyUI/models/vae/wan_2.1_vae.safetensors`）は既存の Wan 2.2 用資産を継続利用します。`uv run python -m automation start-server` でサーバーを起動し、検証は `uv run python -m automation wan_template_passthrough wan_mountain_expedition` をベースラインとします。プリセットは `standard` のみで運用し、解像度やステップ数は `defaults` と `standard` の値を突き合わせて確認します。生成結果は `ComfyUI/output/wan_output_*.mp4` に保存され、履歴 JSON は `automation` モジュールから取得できます。失敗時は `ComfyUI/logs/latest.log` と HTTP レスポンスを確認し、解像度・フレーム数・選択したプリセット設定を突き合わせてください。
